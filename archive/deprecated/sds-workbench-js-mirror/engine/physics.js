export function computeState(x, lambda) {
    const PI = Math.PI;
    const denom = x * x + x + 1;
...
export function computeStateFromMass(lambda, M) {
    const M_nariai = 1 / (3 * Math.sqrt(lambda));
    if (M <= 0 || M >= M_nariai)
        return null;
...
export function computeStateFromDeltaFrac(lambda, deltaFrac) {
    const maxDelta = 1 / 3 - 1e-6;
    if (deltaFrac <= 0 || deltaFrac >= maxDelta)
        return null;
    const d = deltaFrac;
